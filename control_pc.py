...
def left_click(ele):
	mouse.click(button='left', coords=__get_element_postion(ele))
...
def ctrl_wechat():
	...
	left_click(chat_list_element)

	# 3、点击【文件传输助手】进入到聊天页面
	file_helper_element = weixin_pc_window.child_window(title="文件传输助手", control_type="ListItem")
	left_click(file_helper_element)

	# 表情窗口
	emoji_btn = weixin_pc_window.child_window(title="表情", control_type="Button")
	left_click(emoji_btn)
	emoji_panel = weixin_pc_window.child_window(title="EmotionView", control_type="Panel")
	# left_click(emoji_panel)

# 4、获取输入框元素，模拟输入
	edit_element = weixin_pc_window.child_window(title="输入", control_type="Edit")
	left_click(edit_element)
	sleep(2)
	# 输入内容
